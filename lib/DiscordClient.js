...
class DiscordClient {
  ...
  async setActivity(activityData) {
    ...
    try {
      ...
      // Rate limit: minimum 15 seconds between Discord updates.
      // When rate-limited, we store the latest update and restart the
      // timer so only the NEWEST state is sent when the window opens.
      // This prevents stale updates from queueing up serially.
      const now = Date.now();
      if (this._lastActivityUpdate && (now - this._lastActivityUpdate) < 15000) {
        // Always overwrite with the newest state
        this.currentActivity = { clientId, activity: this.validateActivity(activity) };

        // Cancel any pending timer and start a fresh one for the latest update
        if (this._rateLimitTimer) {
          clearTimeout(this._rateLimitTimer);
        }
        const waitTime = 15000 - (now - this._lastActivityUpdate);
        this.onLog('info', `Activity queued (rate limit, sending latest in ${Math.round(waitTime / 1000)}s)`);
        this._rateLimitTimer = setTimeout(async () => {
          this._rateLimitTimer = null;
          if (this.currentActivity) {
            await this._flushQueuedActivity();
          }
        }, waitTime);
      ...
      // Ensure connection
      let client = await this.connect(clientId);
      
      // Try to set activity; if it fails, reconnect and try once more
      try {
        await client.setActivity(validatedActivity);
      } catch (firstError) {
        this.onLog('warn', `Activity failed, reconnecting: ${firstError.message}`);
        await this._cleanupClient();
  ...
  /**
   * Flush the queued activity directly to Discord, bypassing the
   * rate-limit check.  Called only from the rate-limit timer so
   * we know the cooldown has already elapsed.
   */
  async _flushQueuedActivity() {
    const pending = this.currentActivity;
    if (!pending) return;

    try {
      const { clientId, activity } = pending;
      this.onLog('info', 'Flushing queued activity:', JSON.stringify(activity));
      ...
      try {
        ...
        this.onLog('warn', `Queued activity failed, reconnecting: ${firstError.message}`);
        await this._cleanupClient();
        client = await this.connect(clientId);
        await client.setActivity(activity);
      }
      ...
      this._lastActivityUpdate = Date.now();
      this.metrics.activitiesSet++;
      this.onLog('success', 'Queued activity set successfully');
      this.onActivitySet(activity);
    } catch (error) {
      this.metrics.errors++;
      this.onLog('error', `Error flushing queued activity: ${error.message}`);
      this.onError(error);
    }
  }
  ...
  validateActivity(activity) {
    ...
    // Buttons (if supported) -- Discord allows max 2 buttons
  ...
  async clearActivity() {
    ...
    try {
      ...
      // Cancel any queued rate-limited update
      if (this._rateLimitTimer) {
        clearTimeout(this._rateLimitTimer);
        this._rateLimitTimer = null;
      }
      ...
      if (this.isConnected) {
        try {
          ...
          this.onLog('warn', `Clear failed, reconnecting: ${firstError.message}`);
          // Connection might be stale - try reconnecting
          if (this.currentClientId) {
            await this._cleanupClient();
            const client = await this.connect(this.currentClientId);
