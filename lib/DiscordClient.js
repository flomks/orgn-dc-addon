  ...
  async setActivity(activityData) {
    ...
    try {
      ...
      if (this._lastActivityUpdate && (now - this._lastActivityUpdate) < 15000) {
        // Queue the update - it will be sent on next allowed interval
        this.currentActivity = { clientId, activity: this.validateActivity(activity) };
        this.onLog('info', 'Activity queued (rate limit, will update shortly)');
        ...
        if (!this._rateLimitTimer) {
          ...
          this._rateLimitTimer = setTimeout(async () => {
            ...
            if (this.currentActivity) {
              await this.setActivity({ clientId: this.currentActivity.clientId, activity: this.currentActivity.activity });
            }
          }, waitTime);
        }
